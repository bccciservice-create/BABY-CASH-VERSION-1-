...
state.rewardSettings = {gains:[10,25,50,100],pool:5000,dailyText:"Mission du jour bientot disponible",dailyImage:"",media:[],mediaDirection:"forward"};
changed = true;
}
...
cover.addColorStop(0,"#e5e7eb");
cover.addColorStop(.45,"#9ca3af");
cover.addColorStop(1,"#f59e0b");
ctx.fillStyle = cover;
ctx.fillRect(0,0,rect.width,rect.height);
ctx.fillStyle = "rgba(255,255,255,.9)";
ctx.font = "700 18px Arial";
ctx.textAlign = "center";
ctx.fillText("GRATTE ICI",rect.width / 2,rect.height / 2);
...
<small>Gratte vraiment pour gagner des BCC</small>
</div>
...
document.getElementById("scratchCard").addEventListener("pointerdown",(event)=>{
if(!scratchReady){
return;
}
isScratching = true;
...
document.getElementById("scratchCard").addEventListener("pointermove",(event)=>{
if(!isScratching){
return;
}
event.preventDefault();
...
document.getElementById("scratchCard").addEventListener(eventName,()=>{
isScratching = false;
});
});
